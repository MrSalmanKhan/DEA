...
namespace DEA.TermAssessmentControllers
{
                ...
                else if (excelfile.FileName.EndsWith("xls") || excelfile.FileName.EndsWith("xlsx"))
                {
                    ...
                    using (var package = new ExcelPackage(file.InputStream))
                    {
                        ...
                        for (int rowIterator = 2; rowIterator <= noOfRow; rowIterator++)
                        {
                            ...
                            string ClassName = workSheet.Cells[rowIterator, 1].Text.ToString();
                            string SubjectName = workSheet.Cells[rowIterator, 2].Text.ToString();
                            string Catageory = workSheet.Cells[rowIterator, 3].Text.ToString();

                            int ClassID = db.AspNetClasses.Where(x => x.ClassName == ClassName).Select(x => x.Id).FirstOrDefault();
                            int subjectID = db.AspNetSubjects.Where(x => x.ClassID == ClassID && x.SubjectName == SubjectName).Select(x => x.Id).FirstOrDefault();
                            int catageoryID = db.AspNetAssessment_Questions_Category.Where(x => x.CategoryName == Catageory).Select(x => x.Id).FirstOrDefault();
