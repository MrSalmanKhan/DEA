...
namespace DEA.TermAssessmentControllers
{
    public class TermAssessmentController : Controller
    {
        ...
        public ActionResult ClassEvaluations(string StudentId, string type, string TermId)
        {
            ...
            var result1 = new { ClassId= ClassId, parentComent =parentComent,teacherComent=teacherComent,prinpipalComent=prinpipalComent, status = "success", Value = subjects, TId = TId, role = type, studentdata= studentdata, dinterm = dinterm };
        ...
        public ActionResult subqueEvaluations(string StudentId, string SubId, string TermId)
        {
            ...
            if (cgjhk > 0)
            {
                var result1 = new { status = "success", Value = sessionassesques, TId = secnumber, ClassId = ClassId };
                return Json(result1, JsonRequestBehavior.AllowGet);
            }
            else
            {
                ...
                var result1 = new { status = "success", Value = sessionassesrtques, TId = secnumber, ClassId = ClassId };
                return Json(result1, JsonRequestBehavior.AllowGet);
            }
