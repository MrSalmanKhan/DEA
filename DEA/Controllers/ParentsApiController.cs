...
namespace DEA
{
    public class ParentsApiController : ApiController
    {
        ...
        //{
        //    var diary = db.AspNetHomeworks.Where(x => x.StudentID == ChildId).Select(x => new { x.Id, x.ParentComment, x.TeacherComment, x.Status, x.Date, x.AspNetUser.Name }).OrderByDescending(x => x.Date).ToList();
        ...
        //{
        //    var diary = db.AspNetHomeworks.Where(x => x.StudentID == ChildId && x.Date == Date).Select(x => new { x.Id, x.ParentComment, x.TeacherComment, x.Status, x.Date, x.AspNetUser.Name }).OrderByDescending(x => x.Date).ToList();
        ...
        //    var down = db.AspNetHomewrok_HomeworkBreakDown.Where(x => x.HomeworkId == Diary.Homework.Id).Select(x=> new {x.Id, x.AspNetHomeworkBreakDown.HomeWork, x.AspNetHomeworkBreakDown.SubjectId }).ToList();
        ...
        //        DOWN.SubjectWork = item.HomeWork;
        //        Diary.DairyDetail.Add(DOWN);
        //    }

        //    DairyDetail DOWN1 = new DairyDetail();
        //    DOWN1.SubjectName = "Reading";
        //    DOWN1.SubjectWork = Diary.Homework.Reading;
        //    Diary.DairyDetail.Add(DOWN1);

        //    Diary.Name = db.AspNetUsers.Where(x => x.Id == Diary.Homework.StudentID).Select(x => x.Name).FirstOrDefault();
