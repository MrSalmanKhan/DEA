...
namespace DEA.ParentController
{
    public class Parent_DashboardController : Controller
    {
        ...
        public JsonResult CalendarNotification()
        {
            ...
            var result = new { PTMlist,PROlist};
        ...
        public JsonResult StudentHomeWork()
        {
                ...
                var homework = (from student_homework in db.AspNetStudent_HomeWork
                            ...
                            select new { student_homework.TeacherComments, student_homework.AspNetHomework.Id, student_homework.Reading, student_homework.AspNetHomework.Date }).ToList();


            var rtu = homework.OrderByDescending(p => p.Date).ToList();
          
            return Json(rtu, JsonRequestBehavior.AllowGet);
        }
        public ActionResult DiaryDetails(int? id)
        {
            var aspNetSubject_Homework = db.AspNetSubject_Homework.Where(x => x.HomeworkID == id).Include(a => a.AspNetSubject);
             ...
             public ActionResult SeenByParent(int HomeWorkId)
        {
            ...
            var obj = db.AspNetStudent_HomeWork.Where(x => x.StudentID == StudentID && x.HomeworkID == HomeWorkId).FirstOrDefault();
